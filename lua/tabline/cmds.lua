...
local function get_icons()
  return vim.tbl_keys(require('tabline.setup').icons)
end
...
local function complete(_, c, _) -- {{{1
  ...
  local res = type(completion[subcmd]) == 'function' and completion[subcmd]()
    or completion[subcmd]
  if arg and res then
    return filter(res, function(_, str)
      return string.find(str, '^' .. arg)
    end)
  elseif subcmd and res then
    return res
  elseif subcmd then
    return filter(subcmds, function(_, str)
      return string.find(str, '^' .. subcmd)
    end)
  else
    return subcmds
  end
...
local function change_mode(arg) -- Change mode {{{1
  ...
  if not cur then
    ...
    if
      old == 'auto' and (h.tabs_mode() == wastab or h.buffers_mode() == wasbuf)
    then
...
local function toggle_filtering(bang, args) -- Toggle filtering {{{1
  ...
  vim.cmd(
    'redraw! | echo "buffer filtering turned '
      .. (s.filtering and 'on' or 'off')
      .. '"'
  )
end
...
local function name_buffer(bang, args) -- Name buffer {{{1
  if (#args == 0 and not bang) or not g.buffers[bufnr()] then
    return
  end
...
local function purge(wipe) -- Purge {{{1
  ...
  for _, buf in ipairs(fn.tabpagebuflist(fn.tabpagenr())) do
    ...
    local nofile = fn.getbufvar(buf, '&buftype') ~= ''
      and fn.getbufvar(buf, '&modified') == 0
...
local function info(bang) -- Info {{{1
  if not bang then
    ...
    print(
      'recent: '
        .. vim.inspect(
          s.filtering and g.recent[getcwd()] or g.recent.unfiltered
        )
    )
    print(
      'order: '
        .. vim.inspect(s.filtering and g.order[getcwd()] or g.order.unfiltered)
    )
  else
    print('--- BUFFERS ---')
